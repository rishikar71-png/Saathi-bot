...
| 3 Apr 2026 | Module 15 complete. Full 16-group test suite run against live Railway bot. Groups 1–13 all pass (Group 13 required fix: greeting handler intercepting mid-session returns — fixed with `len(_session_history) < 4` guard + `_original_text` to prevent targeted prompt saving to history). Group 14 skipped — evening ritual test requires waiting for scheduled time, not practical in test session; will verify during pilot. Group 15 PASS. Group 16 FAIL on two counts: (1) DeepSeek switched to Hindi during vulnerability disclosure despite English user — root cause: language priming had "unless you write in another language first" escape hatch; fixed by removing it and adding explicit language-lock-during-emotional-moments rule to system prompt. (2) Over-probing on vulnerability — DeepSeek asked multi-part excavation question; fixed by adding Rule 4B-i to system prompt: vulnerability disclosure = one plain acknowledgement, no multi-part probe. Group 16 re-tested — still failing. Fixes in deepseek.py are committed but DeepSeek is still ignoring both the language-lock and the no-excavation rule. Needs deeper intervention next session. | adminreset does not reset users table row — only clears diary/memories/health/session logs. This is correct behaviour (preserves check-in times, language, persona for testing). Language priming escape hatch was a latent bug introduced in Module 3 — now closed. Vulnerability no-excavation rule is a new explicit constraint; previously covered only implicitly by Rule 4B and 4H. | **OPEN ISSUE — start next session here:** Group 16 fix not working. Investigate why DeepSeek is ignoring language-lock and 4B-i during emotional vulnerability. May need hardcoded pre-processing (detect vulnerability signal → prepend language-enforcement instruction to user message before DeepSeek call). After Group 16 fixed: start Module 16 (300+ memory question bank). |
