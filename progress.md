...
|---|---|---|
| 21 Mar | — | — |
| 29 Apr | Greeting handler addressing — bypassed Batch 1c address_lock by short-circuiting DeepSeek for short greetings ("hi"/"hello"/"namaste") and using `{name} ji` regardless of `preferred_salutation`. Senior with sal='Ma' got "Good morning, Durga ji." | Threaded `preferred_salutation` through `_get_time_aware_greeting` in main.py; mirrors `rituals._address` semantics. 13/13 unit cases pass incl. regression guard. |
| 29 Apr | Bare-hour parser stored "09:00" as "09:09" — `add_reminder_structured` and `seed_reminders_from_raw` parsed the human-readable reason string with regex; group 2 captured the AM-time HH which got used as MINUTE. | Replaced regex-on-reason with direct `parse["time_24h"]` use at both sites in reminders.py. The 65/65 unit tests in CHECKPOINT.md never tested this integration boundary. |
...
| 1 May | Bug E1''''' — cricket schedule live-scrape attempts (ESPN, Cricbuzz) all failed because match data is JS-hydrated post-SSR. ESPN: 200 from Mac IP, 403 from Railway (Cloudflare ASN block). Cricbuzz: 200 from Railway but matchInfo JSON is React Server Components stream — only 1 occurrence in curl response vs 84 in browser save. | Pivoted to static `data/ipl_2026_schedule.json` (70 league-phase matches + 12 doubleheader days) extracted from one-off Cricbuzz IPL series page browser-save. New `scripts/refresh_ipl_schedule.py` regenerates the JSON whenever Rishi saves a fresh Cricbuzz HTML. `apis.py` -111 LoC (drop scrape code, add `_load_ipl_schedule` + `_format_ipl_lookup(date_iso)` + `lookup_today_ipl_match` thin wrapper). `main.py` -97 LoC (swap import + call site, simplify /cricdebug second-message probe). 6/6 V4 tests pass incl. doubleheader handling. Live-verified: `aaj cricket hai kya?` → bot reply correctly mentions today's RR vs DC, 7:30 PM, Jaipur. Refresh ritual: save Cricbuzz IPL series page → `python3 scripts/refresh_ipl_schedule.py <path>` → commit JSON → push. Schedule-cache held in process for lifetime; refreshes need Railway redeploy. |
